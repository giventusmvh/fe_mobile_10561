...
const HistoryGym = ({ navigation }) => {
  React.useLayoutEffect(() => {
    navigation.setOptions({
      title: "List History Gym Member",
    });
  }, [navigation]);
  ...
  useEffect(() => {
    const fetchHistoryData = async () => {
      try {
        ...
        if (id) {
          axios
            .get(`https://api.gofit.given.website/api/bookingGym/history/${id}`)
            .then((response) => {
  ...
  const renderHistoryItem = ({ item, index }) => (
    <View style={[styles.row, index % 2 === 1 && styles.rowAlternate]}>
      <View style={[styles.column, styles.borderRight]}>
        ...
        <Text>{item.tgl_booking}</Text>
      </View>
      <View style={[styles.column, styles.borderRight]}>
        <Text style={styles.label}>ID Sesi:</Text>
        <Text>{item.id_sesi}</Text>
      </View>
      <View style={[styles.column, styles.borderRight]}>
        <Text style={styles.label}>Jam Mulai:</Text>
        <Text>{item.jam_mulai}</Text>
      </View>
      <View style={[styles.column, styles.borderRight]}>
        <Text style={styles.label}>Jam Selesai:</Text>
        <Text>{item.jam_selesai}</Text>
      </View>
      <View style={styles.column}>
        <Text style={styles.label}>Waktu Presensi Gym:</Text>
        <Text>{item.waktu_presensi_gym === null ? "---" : item.waktu_presensi_gym}</Text>
      </View>
    </View>
  );
